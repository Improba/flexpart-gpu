// WGSL particle advection kernel with dual-time wind field interpolation.
//
// Variant of advection.wgsl for Tier 1.2 optimization: instead of receiving a
// single pre-interpolated wind field each timestep, this kernel receives two
// wind field snapshots (t0 and t1) uploaded once per met bracket. Temporal
// interpolation is performed inline: wind = (1 - alpha) * wind_t0 + alpha * wind_t1.
//
// Numerical scheme is identical to advection.wgsl (Euler predictor + Petterssen
// corrector); only the wind sampling changes.
//
// Buffer contract:
// - binding 0: particle buffer (read_write)
// - bindings 1-3: wind_t0 (u, v, w) storage buffers, read-only
// - bindings 4-6: wind_t1 (u, v, w) storage buffers, read-only
// - binding 7: DualWindAdvectionParams uniform (includes alpha)
struct Particle {
    pos_x: f32,
    pos_y: f32,
    pos_z: f32,
...
// Trilinear interpolation of a single component from both time steps, with
// temporal lerp. Each component pair (t0, t1) is sampled and blended by alpha.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
    ...
    let u0 = sample_dual_u(x0, y0, z0) + particle.turb_u;
    let v0 = sample_dual_v(x0, y0, z0) + particle.turb_v;
    let w0 = sample_dual_w(x0, y0, z0);
