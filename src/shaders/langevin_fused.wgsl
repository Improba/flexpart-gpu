// Fused Hanna + Langevin turbulence kernel.
//
// Computes Hanna PBL turbulence parameters inline from PBL grid fields, then
// applies the Langevin turbulent velocity update — all in a single dispatch.
// This eliminates the intermediate HannaParams buffer (~64 MB for 1M particles)
// and one dispatch barrier compared to the separated Hanna → Langevin path.
//
// Physics is identical to hanna_params.wgsl + langevin.wgsl executed sequentially.
//
// Bindings:
//   @binding(0): particles (read_write)
//   @binding(1): pbl_ustar (read)
//   @binding(2): pbl_wstar (read)
//   @binding(3): pbl_hmix  (read)
//   @binding(4): pbl_oli   (read)
//   @binding(5): uniform FusedLangevinParams
...
@group(0) @binding(5)
var<uniform> params: FusedLangevinParams;
...
fn compute_hanna(ust_in: f32, wst_in: f32, ol_in: f32, h_in: f32, z_in: f32) -> HannaParams {
    ...
    var sigu: f32; var sigv: f32; var sigw: f32;
    var dsigwdz: f32;
    var tlu: f32; var tlv: f32; var tlw: f32;
...
fn hanna_short_w(ust_in: f32, wst_in: f32, ol: f32, h_in: f32, z_in: f32,
    ...
    var sigw: f32; var dsigwdz: f32; var tlw: f32;
...
fn update_horizontal_component(previous: f32, sigma_in: f32, tl_in: f32, dt: f32, eta: f32) -> f32 {
    ...
    if (is_finite_scalar(next)) { return next; }
    return 0.0;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) nwg: vec3<u32>) {
    ...
    // --- PBL lookup: nearest-cell from particle horizontal position ---
    ...
    let h_pbl = pbl_hmix[pbl_idx];
    let ol = obukhov_length_from_inverse(pbl_oli[pbl_idx]);

    // --- Compute full Hanna params inline (replaces separate Hanna dispatch) ---
    ...
    // --- Horizontal turbulence: one full-dt update ---
